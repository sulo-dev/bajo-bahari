...
// 1. Perbaikan pada Interface (Hapus 'any')
interface LayananItem {
  id: number;
  nama_surat: string;
  deskripsi: string;
  syarat: string; // Tipe aslinya dari Supabase adalah string (karena JSON)
  icon: string;
}

export default async function LayananSuratPage() {
  // 1. TARIK DATA LAYANAN SURAT
  const { data: layananData } = await supabase
    .from("layanan_surat")
    .select("*")
    .order("id", { ascending: true });

  // 2. TARIK NOMOR WHATSAPP DARI KONTAK DESA
  ...
  const noWhatsApp = kontakData?.no_whatsapp || "6281234567890"; // Fallback jika belum diatur

  // 3. PARSING DATA PERSYARATAN (string -> string[])
  const daftarLayanan = layananData?.map((item: LayananItem) => {
    let parsedSyarat: string[] = [];
    
    // Perbaikan Logika Parsing agar 100% Type-Safe
    if (item.syarat) {
      try {
        const parsed = JSON.parse(item.syarat);
        if (Array.isArray(parsed)) {
          parsedSyarat = parsed;
        } else {
          parsedSyarat = [item.syarat]; // Jika bukan array, anggap sebagai string tunggal
        }
      } catch (e) {
        parsedSyarat = [item.syarat]; // Fallback jika gagal parse (misal admin ngetik manual bukan array)
      }
    } else {
      parsedSyarat = ["Membawa identitas diri (KTP/KK)"];
    }
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        ...
        {/* Info Alur Pelayanan (Statis karena bersifat edukasi umum) */}
        ...
        {/* Daftar Layanan Surat Dinamis */}
        <div className="mb-12">
          <h2 className="text-3xl font-bold text-bajo-dark mb-10 text-center">Jenis Layanan & Persyaratan</h2>
          
          {daftarLayanan.length === 0 ? (
            <div className="text-center bg-gray-50 py-10 rounded-2xl border border-gray-200 text-gray-500 font-medium">
              Data layanan surat belum ditambahkan oleh Pemerintah Desa.
            </div>
          ) : (
                  ...
                  <div key={layanan.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col h-full">
                    <div className="p-8 flex-grow">
                      <div className="flex items-center gap-4 mb-4">
                        <div className="text-4xl bg-gray-50 w-16 h-16 rounded-xl flex items-center justify-center border border-gray-100 flex-shrink-0">
                          {layanan.icon}
                        </div>
                      ...
                      <div className="bg-bajo-light/10 p-5 rounded-xl border border-bajo-light/30">
                        ...
                        <ul className="space-y-2">
                          {layanan.syarat.map((syarat: string, index: number) => (
                            <li key={index} className="flex items-start gap-2 text-gray-700 text-sm">
                              <span className="text-bajo-secondary mt-0.5 font-bold">•</span>
                              <span>{syarat}</span>
                            </li>
                          ))}
                        </ul>
                    ...
                    <div className="p-6 bg-gray-50 border-t border-gray-100 mt-auto">
                      <a 
                        ...
                        className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-xl transition-colors flex justify-center items-center gap-2 shadow-sm"
                      >
